    ...
    <div className="animate-fade-in-up space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Danh sách tất cả thông báo dành cho phụ huynh và học sinh</p>
        </div>
              ...
              {notifications.length > 0 ? (
                notifications.map((notif) => (
                  <div
                    ...
                    className={`p-4 hover:bg-gray-50 transition-all cursor-pointer group flex items-start gap-4 ${selectedNotif?.id === notif.id ? 'bg-indigo-50/50 border-l-4 border-indigo-500' : 'border-l-4 border-transparent'
                      }`}
                  >
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-transform group-hover:scale-110 ${notif.isRead ? 'bg-gray-100 text-gray-400' : 'bg-indigo-100 text-indigo-600'
                      }`}>
                      <Bell className="w-5 h-5" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className={`text-sm ${notif.isRead ? 'text-gray-700 font-medium' : 'text-gray-900 font-bold'} truncate group-hover:text-indigo-600`}>
                          {notif.title}
                        </h3>
                    ...
                    <ChevronRight className={`w-4 h-4 mt-1 transition-colors ${selectedNotif?.id === notif.id ? 'text-indigo-500' : 'text-gray-300'}`} />
                  </div>
                ))
              ) : (
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden sticky top-24">
              ...
              <div className="p-8">
                <div className="flex items-center gap-4 border-b border-gray-100 pb-6 mb-6">
                  <div className="w-14 h-14 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 text-xl font-bold uppercase shadow-inner">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                      ...
                      <div className="flex items-center gap-1.5 text-indigo-600 bg-indigo-50 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider">
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-gray-800 text-base leading-relaxed whitespace-pre-wrap bg-slate-50/50 p-6 rounded-2xl border border-slate-100 shadow-inner">
