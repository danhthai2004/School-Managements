...
import {
    ...
    Calculator, X, HelpCircle, ChevronDown, ChevronRight, BookOpen
} from "lucide-react";
import { teacherService } from "../../../services/teacherService";
import type {
    GradeBook, StudentGrade, AssignedClass, SubGradeColumn, SubGradeValue,
    TeacherProfile, HomeroomGradeSummary
...
const GradesPage = () => {
    ...
    const selectedAssignment = useMemo(() =>
        assignedClasses.find(ac => ac.classId === selectedClassId && ac.subjectId === selectedSubjectId),
        [assignedClasses, selectedClassId, selectedSubjectId]
    );
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                    <nav className="flex -mb-px">
                        <button
                            ...
                            className={`px-6 py-4 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'subject'
                        ...
                        <button
                            ...
                            className={`px-6 py-4 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeTab === 'homeroom'
                ...
                <>
                                    ...
                                    onClick={async () => {
                                        if (selectedClassId && selectedSubjectId) {
                                            try {
                                                ...
                                                await teacherService.exportGradeReport(selectedClassId, selectedSubjectId, selectedSemester.toString(), filename);
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100 flex flex-wrap gap-4 items-end">
                        <div className="flex-1 min-w-[300px]">
                            <label className="block text-sm font-medium text-gray-700 mb-2">Lớp & Môn học</label>
                            <select
                                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none bg-white shadow-sm"
                        ...
                        <div className="w-48">
                            ...
                            <div className="flex bg-gray-100 rounded-lg p-1 shadow-inner">
                                <button
                                    ...
                                    className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${selectedSemester === 1 ? 'bg-white shadow-sm text-blue-700 border border-gray-200' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    HK I
                                </button>
                                <button
                                    onClick={() => setSelectedSemester(2)}
                                    className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${selectedSemester === 2 ? 'bg-white shadow-sm text-blue-700 border border-gray-200' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    HK II
                                </button>
                            </div>
                        ...
                        <div className="w-48">
                            ...
                            <div className="flex bg-gray-100 rounded-lg p-1 shadow-inner">
                                <button
                                    ...
                                    className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${sortBy === 'NAME' ? 'bg-white shadow-sm text-blue-700 border border-gray-200' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    Tên
                                </button>
                                <button
                                    onClick={() => setSortBy('CODE')}
                                    className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${sortBy === 'CODE' ? 'bg-white shadow-sm text-blue-700 border border-gray-200' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    Mã HS
                                </button>
                            </div>
                        ...
                        <div className="ml-auto flex items-center gap-3">
                            {msg && (
                                <div className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm shadow-sm animate-in fade-in slide-in-from-top-2 ${msg.type === 'success' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-red-50 text-red-700 border border-red-100'}`}>
                    ...
                    {gradeBook && hasOverflow && (
                        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex gap-3 text-amber-700 animate-in fade-in slide-in-from-left-2">
                            <HelpCircle className="w-5 h-5 flex-shrink-0 mt-0.5" />
                            <div>
                                <h4 className="font-medium">Cấu hình điểm thành phần</h4>
                                <p className="text-sm mt-1">
                                    Có {totalSubGradeColumns} đầu điểm thành phần nhưng chỉ có {regularCount} cột ĐĐGtx chính thức.<br />
                                    Nhấn vào biểu tượng <strong>?</strong> tại mỗi cột ĐĐGtx để chọn cách tính từ các điểm thành phần.
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                        {!selectedClassId ? (
                            <div className="p-20 text-center text-gray-500 bg-gray-50/30">
                                <Calculator className="w-16 h-16 mx-auto text-gray-300 mb-4 opacity-50" />
                                <h3 className="text-lg font-medium text-gray-900">Tính năng quản lý điểm</h3>
                                <p className="max-w-xs mx-auto text-sm">Vui lòng chọn lớp và môn học phía trên để bắt đầu nhập liệu và quản lý điểm số học sinh.</p>
                            </div>
                        ) : loading ? (
                            <div className="p-20 text-center bg-gray-50/30">
                                ...
                                <p className="mt-4 text-gray-500 font-medium">Đang tải dữ liệu bảng điểm...</p>
                            </div>
                        ) : !gradeBook ? (
                            <div className="p-20 text-center text-gray-500 bg-gray-50/30">
                                <AlertCircle className="w-16 h-16 mx-auto text-red-300 mb-4 opacity-50" />
                                <h3 className="text-lg font-medium text-gray-900">Lỗi dữ liệu</h3>
                                <p className="max-w-xs mx-auto text-sm">{msg?.type === 'error' ? msg.text : "Không thể tải dữ liệu bảng điểm từ hệ thống."}</p>
                            </div>
                        ) : (
                            <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-gray-200">
                                <table className="w-full text-sm text-left border-collapse">
                                    <thead className="bg-gray-50/80 backdrop-blur-sm text-gray-700 uppercase font-bold border-b-2 border-gray-200">
                                        <tr className="divide-x divide-gray-200">
                                            <th className="px-3 py-4 w-12 text-center sticky left-0 bg-gray-50 z-20">#</th>
                                            <th className="px-4 py-4 w-32 sticky left-12 bg-gray-50 z-20">Mã HS</th>
                                            <th className="px-4 py-4 min-w-[180px] sticky left-[11rem] bg-gray-50 z-20 shadow-[2px_0_5px_rgba(0,0,0,0.05)]">Họ và tên</th>

                                            {/* Sub-Grade Columns */}
                                            {subGradeColumns.length > 0 && (
                                                <th className="px-2 py-3 text-center bg-emerald-50/80 border-l-2 border-emerald-200" colSpan={subGradeColumns.length}>
                                                    <div className="flex items-center justify-center gap-2">
                                                        <span className="text-[11px] text-emerald-800 tracking-wider">Điểm thành phần (ĐTP)</span>
                                                        {gradeBook.canEdit && <AddColumnDropdown onAdd={handleAddColumn} />}
                                                    </div>
                                                </th>
                                            )}
                                            {subGradeColumns.length === 0 && gradeBook.canEdit && (
                                                <th className="px-4 py-3 text-center bg-emerald-50/80 border-l-2 border-emerald-200">
                                                    <div className="flex items-center justify-center gap-2">
                                                        <span className="text-[11px] text-emerald-800 tracking-wider uppercase">ĐTP</span>
                                            ...
                                            {/* Regular Columns */}
                                            <th className="px-4 py-3 text-center bg-blue-50/80 border-l-2 border-blue-200" colSpan={regularCount}>
                                                <span className="text-[11px] text-blue-800 tracking-wider">Đánh giá thường xuyên (HS1)</span>
                                            </th>
                                            <th className="px-4 py-4 w-28 text-center bg-purple-50/80 border-l-2 border-purple-200 text-[11px] text-purple-800">Giữa kỳ (HS2)</th>
                                            <th className="px-4 py-4 w-28 text-center bg-orange-50/80 border-l-2 border-orange-200 text-[11px] text-orange-800">Cuối kỳ (HS3)</th>
                                            <th className="px-4 py-4 w-24 text-center border-l-2 border-indigo-200 bg-indigo-50/80 text-[11px] text-indigo-800">TBM</th>
                                            <th className="px-4 py-4 w-32 text-center text-[11px] text-gray-600">Ghi chú</th>
                                        </tr>
                                        <tr className="bg-gray-100/50 border-b divide-x divide-gray-200">
                                            <th colSpan={3} className="sticky left-0 bg-gray-100 z-10 shadow-[2px_0_5px_rgba(0,0,0,0.05)]"></th>
                                            {subGradeColumns.map(col => (
                                                <th key={`${col.category}-${col.subIndex}`} className="px-1 py-1.5 text-center bg-emerald-50/20">
                                                    <div className="flex items-center justify-center gap-1 group/col">
                                                        <span className="text-[10px] font-bold text-emerald-600">{col.label}</span>
                                                        {gradeBook.canEdit && (
                                                            <button
                                                                onClick={() => handleRemoveColumn(col.category, col.subIndex)}
                                                                className="opacity-0 group-hover/col:opacity-100 text-gray-400 hover:text-red-500 transition-all p-0.5 rounded-full hover:bg-red-50"
                                                                title="Xóa cột"
                                                            >
                                                                <X className="w-3 h-3" />
                                                            </button>
                                            ...
                                            {subGradeColumns.length === 0 && gradeBook.canEdit && <th className="bg-emerald-50/20"></th>}

                                            {Array.from({ length: regularCount }).map((_, i) => (
                                                <th key={`reg-label-${i}`} className="px-1 py-1.5 text-center bg-blue-50/20">
                                                    <div className="flex items-center justify-center gap-1">
                                                        <span className="text-[10px] font-bold text-blue-600 uppercase">ĐĐG{i + 1}</span>
                                                        {hasOverflow && gradeBook.canEdit && (
                                                            <button
                                                                onClick={() => setResolvePopup(i + 1)}
                                                                className="p-1 text-amber-500 hover:text-amber-700 hover:bg-amber-50 rounded-full transition-all"
                                                                title={`Tính điểm ĐĐG${i + 1} từ ĐTP`}
                                                            >
                                            ...
                                            <th className="bg-purple-50/20"></th>
                                            <th className="bg-orange-50/20"></th>
                                            <th className="bg-indigo-50/20"></th>
                                            <th className="bg-gray-50/20"></th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100">
                                        {processedStudents.map((student, idx) => {
                                            ...
                                            return (
                                                <tr key={student.studentId} className="hover:bg-blue-50/30 transition-colors group divide-x divide-gray-50">
                                                    <td className="px-3 py-2 text-center text-gray-400 font-medium sticky left-0 bg-white group-hover:bg-blue-50/30 transition-colors z-10">{idx + 1}</td>
                                                    <td className="px-4 py-2 text-xs font-mono text-gray-500 sticky left-12 bg-white group-hover:bg-blue-50/30 transition-colors z-10">{student.studentCode}</td>
                                                    <td className="px-4 py-2 font-bold text-gray-900 sticky left-[11rem] bg-white group-hover:bg-blue-50/30 transition-colors z-10 shadow-[2px_0_5px_rgba(0,0,0,0.02)]">
                                                    ...
                                                    {subGradeColumns.map(col => {
                                                        ...
                                                        return (
                                                            <td key={`sg-${col.category}-${col.subIndex}`} className="px-1 py-2 text-center bg-emerald-50/10">
                                                    ...
                                                    {subGradeColumns.length === 0 && gradeBook.canEdit && (
                                                        <td className="px-1 py-2 text-center bg-emerald-50/10"><span className="text-gray-300 text-xs">—</span></td>
                                                    )}

                                                    {Array.from({ length: regularCount }).map((_, i) => {
                                                        const regValue = getGradeValue(student, 'REGULAR', i + 1);
                                                        return (
                                                            <td key={`reg-${i}`} className="px-1 py-2 text-center bg-blue-50/10">
                                                                <div className="flex justify-center">
                                                                    <GradeInput
                                                                        value={regValue}
                                                    ...
                                                    <td className="px-1 py-2 text-center bg-orange-50/10">
                                                        <div className="flex justify-center">
                                                            <GradeInput
                                                    ...
                                                    <td className="px-4 py-2 text-center">
                                                        <span className={`text-[13px] font-black ${avg !== '-' ? (parseFloat(avg) >= 5 ? 'text-indigo-700' : 'text-red-500') : 'text-gray-300'}`}>
                                                            {avg}
                                                        </span>
                                                    </td>
                                                    ...
                                                    <td className="px-3 py-2">
                                                        <input
                                                            type="text"
                                                            className="w-full text-[11px] border-0 bg-transparent focus:ring-1 focus:ring-blue-200 rounded p-1 text-gray-500 placeholder:text-gray-300 transition-all font-medium"
                                                            placeholder="Ghi chú..."
                                                        />
                                                    </td>
                                                </tr>
                    ...
                    {!gradeBook?.canEdit && gradeBook && (
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-5 flex gap-4 text-blue-800 shadow-sm animate-in fade-in zoom-in-95">
                            ...
                            <div>
                                <h4 className="font-bold text-blue-900">Chế độ xem bảng điểm</h4>
                                <p className="text-sm mt-1 opacity-90 leading-relaxed">
                                    Bạn đang xem bảng điểm này với vai trò <strong>Giáo viên chủ nhiệm</strong>.
                                    Do đây không phải là môn bạn giảng dạy trực tiếp, quyền chỉnh sửa đã bị tắt.
                                    Mọi thay đổi cần được thực hiện bởi giáo viên bộ môn tương ứng.
                                </p>
                            </div>
                        </div>
                    )}
                    ...
                    <ImportGradesModal
                        ...
                        onSuccess={() => {
                            fetchGrades();
                            setMsg({ type: 'success', text: 'Nhập điểm từ Excel thành công!' });
