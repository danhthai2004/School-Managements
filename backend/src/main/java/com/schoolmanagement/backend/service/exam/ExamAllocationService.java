...
public class ExamAllocationService {
         ...
         * @param academicYear Năm học entity
         * @return Tổng số học sinh đã phân bổ
         */
        @Transactional
        public int allocateStudents(ExamSchedule examSchedule, School school, AcademicYear academicYear) {
                ...
                // 1. Pool: Lấy tất cả học sinh cùng khối VÀ có học môn thi này từ
                // ClassEnrollment
                List<Student> students = poolStudentsByGradeAndSubject(school, academicYear, grade,
                                examSchedule.getSubject());
                if (students.isEmpty()) {
                        throw new ApiException(HttpStatus.BAD_REQUEST,
                                        "Không tìm thấy học sinh nào thuộc khối " + grade + " có học môn "
        ...
        /**
         * Pool tất cả học sinh thuộc 1 khối lớp và có học môn thi này từ
         * ClassEnrollment.
         */
        private List<Student> poolStudentsByGradeAndSubject(School school, AcademicYear academicYear, int grade,
                ...
                // & kiểm tra lớp đó có học môn này không thông qua Combination
                return classEnrollmentRepository.findAll().stream()
                                .filter(ce -> ce.getAcademicYear() != null && ce.getAcademicYear().getId().equals(academicYear.getId()))
                                .filter(ce -> ce.getClassRoom().getSchool().getId().equals(school.getId()))
                                .filter(ce -> ce.getClassRoom().getGrade() == grade)
                                // Filter ensuring the class actually studies the exam subject
                                .filter(ce -> {
                                        com.schoolmanagement.backend.domain.entity.classes.Combination combo = ce
                                                        .getClassRoom().getCombination();
                                        if (combo == null || combo.getSubjects() == null)
                                                return false;
                                        return combo.getSubjects().stream()
